...
export default function ProjectCard({
  ...
  return (
    <div className="flex flex-col gap-[10px]">
      ...
      <Link
        href={projectHref}
        className="press block rounded-[16px] sm:rounded-[21px] overflow-hidden h-[190px] sm:h-[229px] bg-zinc-900"
        aria-hidden="true"
        tabIndex={-1}
      >
        {screenshotUrl ? (
      ...
      <Link
        href={projectHref}
        className="press focusable group self-start"
      >
        <Heading className="text-title text-white group-hover:text-accent-soft transition-colors">
          {title}
        </Heading>
      </Link>
      ...
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          {safeGithubUrl && (
            <a
              ...
              className="press focusable tap inline-flex gap-1.5 text-label text-zinc-400 border border-zinc-500 rounded-full px-2.5 py-0.5 hover:border-accent-soft/40 hover:text-accent-soft"
            >
        ...
        <Link
          href={projectHref}
          className="press focusable tap text-label text-quiet hover:text-white shrink-0"
        >
          Read more →
        </Link>
      </div>
