      ...
      <main className="max-w-[1440px] mx-auto">
        <div className="max-w-[760px] mx-auto px-6 xl:px-0 pt-[88px] sm:pt-[112px] pb-24">
          ...
          <nav className="flex items-center gap-2 mb-6 font-body text-[13px]" aria-label="Breadcrumb">
            ...
            <span className="text-[#a1a1aa]">Blogs</span>
          </nav>

          <hr className="border-[#27272a] mb-6" />

          {blogs.length === 0 ? (
            <p className="font-body text-[15px] text-[#52525b] text-center py-24">No posts yet</p>
          ) : (
                ...
                return (
                  <li key={blog.slug.current} className="flex gap-5 py-6">
                    ...
                    <div className="flex flex-col gap-1.5 min-w-0">
                      <Link
                        href={`/blogs/${blog.slug.current}`}
                        className="font-display text-[17px] text-white leading-snug hover:text-[#d4d4d8] transition-colors"
                      >
                        {blog.title}
                      </Link>

                      {blog.publishedAt && (
                        <p className="font-body text-[12px] text-[#52525b]">{formatDate(blog.publishedAt)}</p>
                      )}

                      {blog.shortDescription && (
                        <p className="font-body text-[14px] text-[#71717a] leading-relaxed line-clamp-2">
                          {blog.shortDescription}
                        </p>
                      )}
